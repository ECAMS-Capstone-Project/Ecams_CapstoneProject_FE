...
import { Edit3, Mail, Phone, School, ShieldCheck, Search } from "lucide-react";
...
import {
  AcceptRepresentativeRequestAPI,
  GetAllRequestByRepresentativeAPI,
  RepChangeRequest,
} from "@/api/admin/RequestRepresentative";
...
const DashboardAdminRepRequestsPage: React.FC = () => {
  const [requests, setRequests] = useState<RepChangeRequest[]>([]);
  const [loading, setLoading] = useState<boolean>(true);
  const [error, setError] = useState<string | null>(null);
  const [selectedRequest, setSelectedRequest] =
    useState<RepChangeRequest | null>(null);
  ...
  useEffect(() => {
    async function fetchRequests() {
      try {
        setLoading(true);
        const response = await GetAllRequestByRepresentativeAPI(
          search,
          statusFilter,
          currentPage,
          ITEMS_PER_PAGE
  ...
  return (
    <div className="p-6">
      ...
      <div className="grid sm:grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 mt-6">
        {loading ? (
          ...
          currentRequests.map((req) => (
            <MagicCard
              ...
              className="cursor-pointer flex flex-col items-center justify-center overflow-hidden rounded-lg shadow-md hover:shadow-xl transition-transform bg-gradient-to-br from-blue-100 to-cyan-100"
              gradientColor="#D1EAF0"
            >
              <Card className="w-full rounded-lg bg-white shadow-lg hover:shadow-xl transition-all duration-300 border border-blue-100">
                <CardHeader className="bg-gradient-to-r from-[#136CB9] to-[#49BBBD] text-white rounded-t-lg py-4 px-5">
            ...
            <div className="space-y-5 text-[16px] text-gray-800">
              <div className="text-center">
                ...
                <p className="text-gray-600">
                  {selectedRequest.universityAddress}
                </p>
              </div>

              <div className="mt-4">
                <strong className="text-blue-600">Requester Info:</strong>
                <p>
                  <strong>{selectedRequest.requesterInfo.fullname}</strong>
                </p>
              ...
              <div className="mt-3">
                ...
                <p>
                  <strong>
                    {selectedRequest.requestedPersonInfo.fullname}
                  </strong>
                </p>
...
export default DashboardAdminRepRequestsPage;
