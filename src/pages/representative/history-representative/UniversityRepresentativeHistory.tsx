...
import {
  GetHistoryChangeRepresentativeAPI,
  GetRepresentativeHistoryDTO,
  RepresentativeChangeInfo,
} from "@/api/representative/RequestChangeRepresentative";
...
const UniversityRepresentativeHistory: React.FC = () => {
  ...
  const [selectedRep, setSelectedRep] =
    useState<GetRepresentativeHistoryDTO | null>(null);
  ...
  const fetchHistory = async () => {
    ...
    try {
      ...
      const response = await GetHistoryChangeRepresentativeAPI(
        user.universityId,
        params
      );
      ...
      <div className="p-6 max-w-7xl mx-auto">
        ...
        {loading ? (
          ...
          <p className="text-center text-gray-500">
            No representative history found.
          </p>
        ) : (
          <>
            <div className="grid md:grid-cols-2 gap-6">
              {history
                ...
                .map((rep) => (
                  <Card
                    key={rep.userId}
                    className="shadow-lg hover:shadow-xl transition-all duration-300 border border-gray-200 rounded-2xl p-4 bg-white"
                  >
                    <CardHeader className="flex items-center gap-5">
                      <img
                        src={
                          rep.avatar ||
                          `https://api.dicebear.com/6.x/initials/svg?seed=${rep.fullname}`
                        }
                        alt={rep.fullname}
                      ...
                      <Grid2 container spacing={2}>
                        <Grid2 size={6}>
                          <div className="flex gap-2">
                            ...
                            <span>{displayDate(rep.startDate)}</span>
                          </div>
                        </Grid2>
                        <Grid2 size={6}>
                          <div className="flex justify-end gap-2">
                            <span className="font-semibold">End Date:</span>
                            <span>{displayDate(rep.endDate)}</span>
                          </div>
                        </Grid2>
                      </Grid2>
                      <Grid2 container spacing={2}>
