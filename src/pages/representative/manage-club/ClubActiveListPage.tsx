...
const ClubActiveListPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const loadClubs = async () => {
      setLoading(true);
      try {
        const response = await GetAllActiveClubsAPI(
          user.universityId ?? "",
          "ACTIVE",
          page,
          pageSize
      ...
      {loading ? (
        ...
        <>
          <div className="flex items-center justify-between pt-4">
            <Heading
              title="View All Club"
              description={`All club active in ${user?.universityName}`}
            />
          </div>
          <Separator />
          <ClubActiveListTable data={clubs} setFlag={setFlag} />
          <DataTablePagination
            currentPage={page}
