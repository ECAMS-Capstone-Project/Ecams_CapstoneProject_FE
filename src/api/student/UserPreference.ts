...
export const UpdateUserPreferenceAPI = async (
  userId: string,
  data: { fieldIds: string[] }
): Promise<ResponseDTO<string>> => {
  try {
    const response = await put<ResponseDTO<string>>(
      `/Students/User/${userId}/favorite`,
      data
    );
...
export const GetRecommendedEventsAPI = async (
  ...
  try {
    ...
    // 2. Append paging at the end
    params.append("PageNumber", pageNumber.toString());
    params.append("PageSize", pageSize.toString());

    const url = `/Event/User/${userId}/recommendation?${params.toString()}`;
...
export const GetUserFavoriteAPI = async (
  userId: string
): Promise<ResponseDTO<FieldDTO[]>> => {
  try {
    const response = await get<ResponseDTO<FieldDTO[]>>(
      `Students/User/${userId}/favorite`
    );
    return response;
  } catch (error) {
