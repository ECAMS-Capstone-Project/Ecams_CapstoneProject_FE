...
export const getPolicyList = async (pageSize: number, pageNo: number): Promise<ResponseDTO<ResponseData<Policy>>> => {
  try {
    const response = await get<ResponseDTO<ResponseData<Policy>>>(`/Policy?PageNumber=${pageNo}&PageSize=${pageSize}`);
...
export const createPolicy = async (value: any): Promise<ResponseDTO<ResponseData<Policy>>> => {
  try {
    const response = await post<ResponseDTO<ResponseData<Policy>>>(`/Policy`, value);
    return response; // Trả về toàn bộ phản hồi
  } catch (error: any) {
    console.error("Error in UniversityList API call:", error.response || error);
    throw error;
