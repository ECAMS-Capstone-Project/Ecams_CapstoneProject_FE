...
export const GetRequestChangeClubOwnerAPI = async (
  universityId: string
): Promise<ResponseDTO<ClubOwnerChangeResponseDTO[]>> => {
  try {
    const response = await get<ResponseDTO<ClubOwnerChangeResponseDTO[]>>(`/Clubs/University/${universityId}/owner-requests`);
...
export const AcceptOrDenyOwnerRequestAPI = async (
  clubId: string,
  data: { rejectReason: string; isAccepted: boolean }
): Promise<ResponseDTO<string>> => {
  try {
    const response = await put<ResponseDTO<string>>(
      `/Clubs/${clubId}/owner-request`,
      data
    );
    return response; // Trả về toàn bộ phản hồi
  } catch (error: any) {
