...
import { AreaPicker, DatePicker } from "../../staff/staff-events/AreaPicker";
...
export const CreateInterClubEvent: React.FC<EventDialogProps> = ({
  ...
  const { createInterClubEvent, isPending } = useEventDetail();
  ...
  const onSubmit = async (values: EventFormValues) => {
    ...
    try {
      ...
      // Trim whitespace from club names before sending
      values.listClubName.forEach((name, index) => {
        formData.append(`ListClubName[${index}]`, name);
      });
      ...
      if (initialData) {
        ...
        await createInterClubEvent(formData);
      }
      navigate(-1);
      if (!isPending) {
