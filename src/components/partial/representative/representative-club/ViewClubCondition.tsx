...
export function ClubConditionView({ clubId, isClubOwner }: Props) {
  ...
  const [selectedCondition, setSelectedCondition] =
    useState<ClubCondition | null>(null);
  const [flag, setFlag] = useState<boolean>(false);

  // State cho confirm dialog khi xóa
  const [confirmOpen, setConfirmOpen] = useState<boolean>(false);
  const [deleteConditionId, setDeleteConditionId] = useState<string | null>(
    null
  );
    ...
    <div className="p-4 space-y-6">
      <div>
        <div className="flex items-center justify-between">
          ...
          {isClubOwner && (
            <ClubConditionCreate setFlag={setFlag} clubId={clubId} />
          )}
        </div>
      </div>
      <Card className="pt-5">
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            {clubCondition && clubCondition.length <= 0 && (
